...
def train():
    ...
    model = WISE_BRCA_d(head_hidden_1=1536, head_hidden_2=768, head_hidden_3=384, dropout=0.40).cuda()
    ...
    for epoch in range(args.epochs):
        ...
        LOSS = []
        model.train()
        progress_bar = tqdm(total=len(data_train_validation_loader), desc="MAIGGT training")
    ...
    model = MAIGGT(head_hidden_1=1536, head_hidden_2=768, head_hidden_3=384, dropout=0.40).cuda()
    ckpt = torch.load(
        f'{args.checkpoint_path}/MAIGGT.pth',
        map_location='cuda:0')
    ...
    print(
        f'Epoch: train AUC: {auc_train_validation},test AUC: {auc_test}')
