...
from model_MAIGGT_mcVAE import MAIGGT_mcVAE
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        if np.mean(LOSS) < min_loss:
            ...
            torch.save(state, args.checkpoint_dir / f'MAIGGT_mcVAE.pth')
    ...
    model = mcVAE().cuda()
    ckpt = torch.load(
        f'{args.checkpoint_path}/MAIGGT_mcVAE.pth',
        map_location='cuda:0')
    model.load_state_dict(ckpt['model'])
    CHCAMS_test_loss = calculate_metrics(model, data_test_loader)
    print(rf'CHCAMS test loss: {CHCAMS_test_loss}')
    print('Finish!')
