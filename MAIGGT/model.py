...
from MAIGGT_mcVAE.utils import Normal, compute_kl, compute_ll, compute_logvar
from MAIGGT_mcVAE.model_WISE_BRCA import WISE_BRCA


class MAIGGT(nn.Module):
    def __init__(self, patho_dim=1536, pheno_dim=96, head_hidden_1=1536, head_hidden_2=768, head_hidden_3=384, WISE_BRCA=WISE_BRCA, dropout=0.4):
        super(MAIGGT, self).__init__()

        self.head_mut = nn.Sequential(nn.Linear(1536, head_hidden_1), nn.Tanh(), nn.Dropout(dropout), nn.BatchNorm1d(head_hidden_1),
        ...
        self.encoder_patho = nn.Sequential(nn.Linear(patho_dim, 2 * patho_dim), nn.LeakyReLU(),
                                           nn.Linear(2 * patho_dim, patho_dim), nn.LeakyReLU(),
                                           nn.Linear(patho_dim, patho_dim // 2))
        self.encoder_pheno = nn.Sequential(nn.Linear(21, 2 * pheno_dim), nn.LeakyReLU(),
                                           nn.Linear(2 * pheno_dim, 4 * pheno_dim), nn.LeakyReLU(),
                                           nn.Linear(4 * pheno_dim, 8 * pheno_dim))
        self.decoder_patho = nn.Sequential(nn.Linear(patho_dim // 2, patho_dim // 2), nn.LeakyReLU(),
                                           nn.Linear(patho_dim // 2, patho_dim), nn.LeakyReLU(),
                                           nn.Linear(patho_dim, patho_dim))
        self.decoder_pheno = nn.Sequential(nn.Linear(8 * pheno_dim, 4 * pheno_dim), nn.LeakyReLU(),
                                           nn.Linear(4 * pheno_dim, 2 * pheno_dim), nn.LeakyReLU(),
                                           nn.Linear(2 * pheno_dim, 21))
