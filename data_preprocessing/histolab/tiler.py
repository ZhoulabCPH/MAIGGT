...
class Tiler(Protocol):
    ...
    def _tile_filename(
        self, tile_wsi_coords: CoordinatePair,tiles_counter: int,slide_name
    ) -> str:
        ...
        x_ul_wsi, y_ul_wsi = tile_wsi_coords
        tile_filename = (
            f"{self.prefix}{slide_name}_tile_{tiles_counter}_level{self.level}_{x_ul_wsi}-{y_ul_wsi}"
...
class GridTiler(Tiler):
    ...
    def extract(
        ...
        for tiles_counter, (tile, tile_wsi_coords) in enumerate(grid_tiles):
            tile_filename = self._tile_filename(tile_wsi_coords, tiles_counter,slide_name=slide.name)
    ...
    def _tiles_generator(
        ...
        dzg = DeepZoomGenerator(slide._wsi, tile_size=self.tile_size[0], overlap=0, limit_bounds=False)

        level_dimensions = dzg.level_count - 1
        level_tiles = dzg.level_tiles[level_dimensions]

        for i in range(level_tiles[0]):
            for j in range(level_tiles[1]):
        # for coords in grid_coordinates_generator:
                try:
                    ...
                    if tile.image.size[0] != self.tile_size[0] or tile.image.size[1] != self.tile_size[1]:
                        continue
