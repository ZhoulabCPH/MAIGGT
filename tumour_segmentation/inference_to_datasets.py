    ...
    with torch.no_grad():
        for slide_name in os.listdir(slides_path):
            ...
            if f'{slide_name}.feather' not in os.listdir(save_path):
                ...
                data = BC_inference(slide_path=rf'{slides_path}/{slide_name}', transform=Transform())
...
if __name__ == '__main__':
    ...
    save_path = rf'../datasets/WSIs_tumor_segmentation_224'
    inference(model, slides_path, save_path)

    model = Tumour_segmentation()
    ckpt = torch.load(
        rf'../checkpoints/Tumour_segmentation_model_512.pth',
        map_location='cuda:0')
    model.load_state_dict(ckpt['model'])
    model.cuda()
    model.eval()
    slides_path = '../datasets/Patches_512'
    save_path = rf'../datasets/WSIs_tumor_segmentation_512'
    inference(model, slides_path, save_path)